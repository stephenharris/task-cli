...
import { getTasks, getTasksWithOrdinal, Task, TaskWithOrdinal } from "../data/tasks";
...
export const listCommmand = (options: any, command: any) => {
    ...
    .then((tasks) => tasks.filter((task: TaskWithOrdinal) => task.status !== 'complete').sort(sortTasks))
    .then((tasks) => {
